                            ...
                            <div className="bg-card border border-border rounded-2xl p-6">
                                ...
                                <div className="space-y-4">
                                    {[
                                        ...
                                        { label: "Page Size", value: `${Math.round(analysis.pageSize / 1024)} KB` },
                                        { label: "Load Time", value: `${(analysis.loadTime / 1000).toFixed(2)}s`, className: analysis.loadTime < 3000 ? "score-good" : analysis.loadTime < 5000 ? "score-medium" : "score-poor" },
                                        { label: "Total Links", value: analysis.links.total },
                                        { label: "Total Images", value: analysis.images.total },
                                        { label: "Total Headings", value: analysis.headings.h1 + analysis.headings.h2 + analysis.headings.h3 + analysis.headings.h4 + analysis.headings.h5 + analysis.headings.h6 },
                                    ].map((item) => (
                                        <div key={item.label} className="flex justify-between items-center p-3 bg-muted/50 border border-border rounded-xl">
                                            <span className="text-sm text-muted-foreground">{item.label}</span>
                                            <span className={`font-bold text-foreground ${item.className || ""}`}>{item.value}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
